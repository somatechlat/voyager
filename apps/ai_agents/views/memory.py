...
def get_memory_entries(request: HttpRequest, agent_id: int, limit: int = 50) -> list[MemoryEntry]:
    """List memory entries for an agent."""
    return list(
        MemoryEntry.objects.filter(agent_id=agent_id, is_active=True).order_by("-importance")[
            :limit
        ]
    )
...
def delete_memory(request: HttpRequest, agent_id: int, qdrant_id: str) -> dict[str, bool]:
...
def consolidate_memory(request: HttpRequest, agent_id: int) -> ConsolidateMemoryResponse:
